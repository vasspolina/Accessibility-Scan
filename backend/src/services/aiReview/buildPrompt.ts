...
- Report what the content does, never where it came from. You cannot tell whether a person, a tool or a model wrote any text on this page, and guessing from style is not evidence. "Image captions describe colours rather than the artwork" is an observation the owner can check; "AI-written captions describe colours" is an accusation they may simply deny, and it wastes the one sentence they read. The same goes for intent: describe the effect on the visitor, not what you suppose the business meant by it.
- Every finding is read on its own. The report groups, sorts and filters them, so there is no previous card guaranteed to sit above yours. Never begin a title with "Another", "Also" or "Similarly", and never refer to a different finding. Two instances of one problem are one finding with a count, not two findings in conversation with each other.
