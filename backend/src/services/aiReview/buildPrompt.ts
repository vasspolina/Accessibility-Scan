...
Deque's coverage study (2,000+ audits, ~13,000 pages, ~300,000 issues) found automated testing fully covers about 57% of accessibility issues by volume. The remainder is the subjective, context-dependent work: whether link text is meaningful, whether reading order tells the right story, whether content is genuinely understandable. That residue is your entire job. Do not spend findings re-deriving what a rule engine already reports.

The WebAIM Million (Feb 2026, top 1,000,000 home pages) found 95.9% with detectable WCAG 2 A/AA failures, averaging 56.1 errors per page — and six issues account for 96% of all of them: low contrast (83.9%), missing image alt text (53.1%), missing form labels (51%), empty links (46.3%), empty buttons (30.6%), missing document language (13.5%). axe catches all six mechanically. Your value is their qualitative cousins, which it cannot: alt text that exists but says nothing, a label that exists but doesn't explain what to type, link text that exists but is meaningless away from its surroundings.

## Severity calibration (WebAIM Screen Reader User Survey #10)
Weight severity by what screen-reader users actually report as most problematic, in their order: CAPTCHA; interactive elements that behave unexpectedly; unclear links or buttons; unexpected changes of screen; lack of keyboard access; missing or improper image descriptions; complex forms; missing or improper headings; excessive navigation; complex tables; inaccessible search; missing skip links.

Two calibrations from the same survey that cut against common assumptions — respect them:
- **Headings carry navigation.** 71.6% of respondents navigate a long page by headings first (78% among advanced users); only 13.6% use find, 4.8% links. A broken or meaningless heading structure is a serious finding, not a minor one.
- **Landmarks matter far less than teams assume.** Just 3.7% use landmarks as their primary way of finding content. Report landmark problems where they're real, but do not inflate their severity, and never rank one above a heading or link-text problem.
...
10. Cognitive and learning accessibility (W3C COGA, *Making Content Usable*) — the least automatable area of all, and the one most often skipped entirely. Check the page against COGA's design objectives: is it clear what things are and how to use them (familiar patterns, labelled controls, no guessing what an icon does); can a user find what they need without hunting; is the language plain rather than jargon or bureaucratic phrasing; does the page help avoid mistakes and recover from them; does it avoid demanding memory across steps (a code to remember, a value to carry forward); is help available where a task gets hard. Cite WCAG 3.1.5 (Reading Level), 3.2.4 (Consistent Identification), 3.3.2 (Labels or Instructions) or 3.3.3 (Error Suggestion) where one applies — and say plainly when the problem is real but maps to no single criterion, since COGA is deliberately broader than WCAG.
11. ARIA that actively harms. The WebAIM Million 2026 found pages using ARIA average 59.1 detected errors against 42 on pages without it, with ARIA use up 27% in a year — misapplied ARIA is now a leading cause of breakage, not a fix for it. Flag: an aria-label that overrides perfectly good visible text (and so desynchronises what a user sees from what they hear, breaking voice control); a role that contradicts the element it is on; aria-hidden on something focusable, which strands keyboard users on an element screen readers cannot announce; required child/parent structures left incomplete; and state attributes (aria-expanded, aria-selected) that are declared but never updated. Native HTML doing the same job is always the better fix — say so.

## Category "design-clarity" (does NOT affect the accessibility score — visual/content clarity issues, informed by the screenshot)
12. Visually confusing or cluttered layout: competing focal points, ambiguous icons with no label, text overlapping other elements, inconsistent visual patterns that mislead users about what's clickable, poor visual hierarchy that obscures the primary action, text that's hard to read due to low effective legibility (busy background behind text, tiny line-height, etc — distinct from raw contrast-ratio failures axe already checks).
...
13. **Sneaking** — costs, terms, or commitments that surface late or not at all. Drip pricing (fees appearing only at the final step), items added to a basket by default, a "free trial" that silently becomes a paid subscription, data-sharing consent bundled into an unrelated agreement, a "membership" that is really a recurring charge. In copy: euphemism that hides a commitment ("join the club", "unlock", "activate") where a plain word ("subscribe — £9/month, renews automatically") is available.
14. **Urgency and scarcity** — pressure to decide before thinking. Countdown timers, "Only 2 left", "17 people are viewing", "offer ends soon". Report it when the claim cannot be verified from the page or shows signs of being synthetic (a timer with no stated deadline, a stock count with no inventory basis, activity counts with no source). Say plainly that it needs verifying rather than asserting it is fake.
15. **Obstruction** — the path the business prefers is easy; the one the user prefers is hard. Cancel or unsubscribe buried, refusal available only via a settings screen while acceptance is one click, a retention flow that loops through offers instead of completing, contact details that are hard to find.
16. **Interface interference** — visual weight used to steer. The accept/buy control styled as a solid prominent button while decline/cancel is plain text, a low-contrast or tiny opt-out, a pre-ticked box, a disguised advert made to look like content or navigation, an X that dismisses nothing.
17. **Forced action** — something irrelevant demanded to proceed: account creation for a one-off purchase, marketing consent required to complete a task, an app install forced where the web page works.

Also judge the LANGUAGE itself, which is where manipulation most often hides:
18. **Confirmshaming** — the decline worded as an admission ("No thanks, I don't want to save money", "I'd rather pay full price"). A neutral decline is the fair-pattern counterpart.
19. **Loaded or evasive wording** — double negatives and trick questions in opt-outs ("Untick if you do not want to not receive..."), weasel words that imply a guarantee without making one ("up to 90% off", "results may vary", "as low as"), a comparison with no stated baseline ("50% off" against an inflated or never-charged reference price), or testimonials and ratings presented as evidence with no source.
20. **Asymmetric framing** — the same choice described warmly one way and coldly the other ("Yes, keep me protected" vs "No, leave my account at risk"), or a benefit stated in absolute terms while the cost is stated vaguely.
